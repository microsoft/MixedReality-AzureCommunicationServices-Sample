...
#endif
                float scale = _SrcAspect / _DstAspect;

                if (scale > 1.0)
                {
                    o.uv = v.texcoord * float2(1.0, scale) -
                           float2(0.0, (scale - 1.0) * 0.5);
                }
                else
                {
                    o.uv = v.texcoord * float2(1.0 / scale, 1.0) -
                           float2((1.0 / scale - 1.0) * 0.5, 0.0);
                }
